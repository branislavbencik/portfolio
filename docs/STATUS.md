...
**Last update:** 2026-04-11 · **Deploy:** branislavbencik.com

## Pages
`/` → Done (landing) · `/skoala` → Done · `/teatime` → Done · `/nnspect` → Done · `/sakurabook` → Done · `/crowdberry` → Done · `/about` → Stub · `/resume` → Not started

## What's Next
About page; resume page; real favicon monogram (current is placeholder); address pre-existing lint warnings; update `CLAUDE.md` (still claims Next 15 + static export)
