...
### Playground / Prototype Video
Playground tiles use the standardized `CaseStudyCard` (no dark-surface exception). When a project sets `thumbnailVideo` in its YAML, the card frame plays a muted demo clip via `<ProjectVideo>`; static tiles are unchanged. The clip's backdrop (off-white field + ~4px on-screen window radius, no shadow) is baked into the mp4 in ScreenKite — CSS can't mat it because mp4 has no alpha. Rendered edge-to-edge; contrast (dark app on off-white) is the only separator. Card label === detail H1 (`title` field, one source of truth).
- **Build-notes detail:** `type: "playground"` builds a full detail page (`generateStaticParams` includes it). Layout: header + hero video → TL;DR (`pt-section`) → `DeliverablesBar` (`pt-detail`) → `<Learnings>` (`pt-detail`), all on a 96px rhythm.
- **Learnings component:** Rich inline content (Geist Mono `.inline-code` chips on `--surface-2` + `--citation-link` links) that the structured Keystatic schema can't hold; lives in `Learnings.tsx`. If a second playground project needs learnings, lift content to a per-slug data module.
- **Cursor:** Playground card passes `cursorTone="inverted"` to `CaseStudyCard` (light pill + hairline border) so the cursor label reads over the dark prototype video.
- **ScreenKite recipe (locked):** Video native 1920×1080, off-white field, 8px native radius (=4px at 0.5× display scale, matching `--radius-sm`). No shadow — contrast alone separates the window.
- **`--playground-*` tokens removed:** dark surface retired; tokens were `--playground-canvas / -border / -text / -text-dim` and their `@theme inline` aliases. Remove any references if they resurface.

### Cursor Label (follow-cursor affordance)
Landing-page cards carry a cursor-follow label that names the click action per card — `"View case study"`, `"View project"`. Layered on top of section labels, not replacing them. The playground tile passes `cursorTone="inverted"` (light pill + hairline) so the label reads over its dark prototype video.
