...
Per project memory: hover gets `scale`/`translate`. No `filter: brightness(...)`, no shadow brightening. Why: brightness reads as cheap; transform reads as physical. The shipped Levitate hover (CaseStudyCard inner scales 0.97 over 320ms) is the canonical pattern.

**Border-color shifts are allowed only when transform isn't viable, and only as supporting hairline.**
Text-only tiles (no raster content to mask the subpixel shift) hit a bind: the no-hover-scale-on-text rule blocks transform on the tile itself, but the tile still needs a hover signal beyond a single icon translate. In that narrow case, a calibrated border-color swap is permitted — but the icon/label transform stays the **primary** affordance, and the border shift is **supporting hairline only** (max one tier on the border ramp, e.g. `--border-light` → `--border-light-hover`). Live use: `<DeliverablesBar />` (border Zinc 200 → Zinc 400 + arrow translate `(2px, -2px)`, 200ms `ease-out`, see `docs/DESIGN.md` § Deliverables Bar). Not a license to revive border-color flashes on cards/tiles that **could** have used transform — exhaust the transform path first.
